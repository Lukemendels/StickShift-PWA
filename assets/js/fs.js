...
async function buildIndexBundle(){
  ...
  let body="";
  for(const p of foundation) body+=bundleAnchor(p,files.get(p),"foundation");
  for(const p of indexes) body+=bundleAnchor(p,files.get(p),"map");
...
async function buildBundleFromRequest(req){
  ...
  const seeds=req.seeds.filter(p=>files.has(p));
  let body="",f=0,m=0,s=0;
